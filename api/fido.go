...
func (s *FidoService) RegisterChallenge(c *gin.Context) {
	...
	creation, webAuthnSession, err := s.W.BeginRegistration(user)
	if err != nil {
...
func (s *FidoService) Register(c *gin.Context) {
	...
	// TODO: find out the reason to encode challenge
	...
	if err := db.CreateCredential(dbConn, db.NewUserCredential(email, credential)); err != nil {
